...
/**
 * 학생용 프리젠테이션 편집기 placeholder — admin과 동일 layout, 권한만 차이.
 */

import { useCallback, useEffect, useState } from "react";
import Link from "next/link";
import { useParams, useRouter } from "next/navigation";
import { ArrowLeft, Plus, Trash2, Presentation, ChevronUp, ChevronDown } from "lucide-react";
...
export default function StudentCourseDeckEditorPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setDeck(d);
      if (d.slides[0] && activeSlide === null) setActiveSlide(d.slides[0].id);
    } catch (e: any) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[200px_1fr] gap-4">
        <aside className="bg-bg-primary border border-border-default rounded-lg p-2 space-y-1 max-h-[80vh] overflow-y-auto">
          <div className="flex items-center justify-between px-2 py-1 mb-1">
            ...
            {canWrite && (
              <button onClick={addSlide} className="p-1 text-accent hover:bg-accent-light rounded">
                <Plus size={14} />
              </button>
            )}
          </div>
          {deck.slides.map((s, i) => {
            ...
            return (
              <div key={s.id} className={`group flex items-stretch gap-1 ${isActive ? "ring-2 ring-accent" : ""} rounded`}>
                <button
                  type="button"
                ...
                {canWrite && (
                  <div className="flex flex-col gap-0.5 opacity-0 group-hover:opacity-100 px-1 py-1">
                    <button onClick={() => moveSlide(i, -1)} disabled={i === 0} className="text-text-tertiary hover:text-accent disabled:opacity-30 p-0.5">
                      <ChevronUp size={11} />
                    </button>
                    <button onClick={() => moveSlide(i, 1)} disabled={i === deck.slides.length - 1} className="text-text-tertiary hover:text-accent disabled:opacity-30 p-0.5">
                      <ChevronDown size={11} />
                    </button>
                    <button onClick={() => deleteSlide(s.id)} disabled={deck.slides.length <= 1} className="text-text-tertiary hover:text-status-error disabled:opacity-30 p-0.5">
                      <Trash2 size={11} />
                    </button>
                  </div>
                )}
