...
/**
 * 자유 산출물 탭 — 학생 본인이 자유롭게 업로드/관리하는 산출물.
 *
 * - 카테고리 필터 + 추가/수정 form (파일 업로드 + 외부 링크 + 태그 + 공개 토글)
 * - 공개로 표시하면 학생 산출물 갤러리 + 다른 학생에게 노출
 */

import { useCallback, useEffect, useRef, useState } from "react";
import {
  Edit3, Eye, EyeOff, FileText, Folder, Globe,
...
function ArtifactCard({
  a, onEdit, onRemove, onTogglePublic,
}: {
  a: Artifact;
  onEdit: (a: Artifact) => void;
  onRemove: (id: number) => void;
  onTogglePublic: (a: Artifact) => void;
}) {
