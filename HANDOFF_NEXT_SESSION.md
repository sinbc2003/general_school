...
> (갱신: 2026-06-12 심야 — **실시간 투표·워드클라우드(Mentimeter형) 완성, 커밋 9b0aa3e push.**
> ⚠️ **B 배포 미완** — B가 06-12 13:30경 또 꺼짐 (아래 §0-2). B 복귀 시:
> `gs-deploy-a.sh` → `gs-deploy-b.sh` (alembic `e0b2d4f6a8c0` 포함) 후
> 스모크 `POST /api/tools/poll` 미인증 403 확인.)

---

## 0-2. ⚠️ B 서버 재다운 (2026-06-12 13:30경~) — 미해결

- 06-12 11:41 부팅 → 13:10 Kahoot 패리티 배포 정상 → **13:30경 다시 offline**
  (Tailscale last seen + pubedu.com 530). 24시간 안에 2번째 비정상 다운.
- 어젯밤과 동일 패턴(§0 참조)이면 전원/망 문제. **2회 반복이므로 하드웨어
  (전원 어댑터 헐거움·과열·절전 BIOS 설정) 의심** — B 앞에서 확인 필요:
  `last reboot` + `journalctl --boot=-1 | tail` + 어댑터 연결 상태.
- B 복귀 시 배포 대기 중: 커밋 9b0aa3e (투표 도구, alembic `e0b2d4f6a8c0`).
...
1. **투표 v2** — 클래스룸 첨부 type=poll(퀴즈 live_quiz 패턴), 척도(1~5)·열린 응답
   질문 타입, 결과 Excel 내보내기, 진행 중 세션 settings 토글(결과 공개 on/off).
   (v1 = 객관식 투표·워드클라우드·PIN 입장·실시간 그래프·드라이브 통합 —
   2026-06-12 완료, 커밋 9b0aa3e, `/tools/poll` + `/s/poll/[pin]`)
