...
- [ ] **스토리지 장비** (아래 §1.5 가이드 참고) — 최소 외장 SSD 4TB 1개
- [ ] **유선 LAN 케이블** (Wi-Fi 비추 — 끊김 위험)
- [ ] **UPS** (선택, 5~10만원짜리도 정전 1회 막음)
- [ ] **본인 노트북** (원격 지원·디버깅용)

### 1.5 스토리지 장비 선택 가이드 (1500명 1년 운영 기준)

**실 사용량 추정**: 300~600GB (할당 1.8TB 중 10~30%). 본체 SSD 256GB만으로는 부족 — 외부 스토리지 필수.

| 옵션 | 비용 | 셋업 | 운영 부담 | 데이터 안전 | 추천 시나리오 |
|---|---|---|---|---|---|
| 외장 USB SSD 4TB 1개 | ~20만원 | 5분 | 최저 | 단일 (백업 ZIP 별도 보관) | 예산 부족 |
| **외장 SSD 4TB ×2 (active + 백업)** | **~40만원** | **10분** | **최저** | **2중 (월 1회 미러)** | **표준 권장** |
| **2-bay NAS + HDD 4TB×2 (RAID 1)** | **~50만원** | **1시간** | **낮음 (NAS UI = 윈도우 탐색기)** | **RAID 1 미러 — 디스크 1개 망가져도 OK** | **안전 + 운영 편함** |
| 4-bay NAS + HDD 4TB×4 (RAID 5/6) | ~100만원 | 1.5시간 | 낮음 | 1~2개 디스크 망가져도 OK | 미래 확장 |
| ~~구형 노트북 5대 클러스터~~ | 0 (재활용) | 1~2일 | **매우 높음** | 분산 (운영 잘하면) | **권장 X** |

### 왜 5대 노트북 클러스터는 권장 X

- 폐 노트북 = 이미 사용된 SSD/HDD 평균 수명 짧음
- NFS 셋업 = Linux 명령어 지식 필요 (코딩 모르는 교사 운영 불가)
- 노트북 1대 꺼지면 NFS unmount stuck → **운영 중 데이터 손실 가능**
- 직접 미러 안 됨 (RAID 1은 직접 연결만)
- 전기료 = NAS 1대보다 많음 (5대 24/7)
- **대안**: 폐 노트북 5대는 학생용 / 처분 → NAS 1대 구매

### 권장 시나리오

**A. 예산 0~30만원 (학교 첫 도입)**: 외장 SSD 4TB 1개 + 백업본 SSD 1개 (월 1회 미러)
**B. 예산 30~50만원 (1년차 안정화)**: Synology DS220+ + WD Red HDD 4TB×2 (RAID 1)
**C. 예산 50만+ (장기)**: Synology DS920+ 4-bay (RAID 5/6) + 외장 SSD 백업본

→ 셋업 명령은 [production/README.md](./production/README.md) §스토리지 셋업 절차 참고.
