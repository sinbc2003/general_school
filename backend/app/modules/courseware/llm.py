...
from app.services.llm_grader import grade_set_batch, resolve_grader_model
...
async def llm_grade_preview(
    ...
    eligible_attempts × (input/output 평균 토큰) × 단가.
    """
...
async def llm_grade_batch(
    ...
    await log_action(
        ...
        detail=(
            ...
            f"failed:{result['failed']} cost:${result['total_cost_usd']:.4f}"
        ),
        is_sensitive=True,
        request=request,
    )
